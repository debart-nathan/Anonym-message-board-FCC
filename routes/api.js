...
async function findBoard(board,res) {
  try {
...
async function threadsPOST(req, res) {
  ...
  let boardData = await findBoard(board,res);
...
async function threadsGET(req, res) {
  ...
  threads = threads.map(function (e) {
    const { _id, text, created_on, bumped_on } = e;
    const replies = e.replies
      .slice(-1 * Math.min(e.replies.length, 3))
      .map(function (e) {
        const { _id, text, created_on } = e;
        return {
          _id,
          text,
          created_on,
...
async function replyGET(req, res) {
  ...
  const { _id, text, created_on, bumped_on } = targetThread;
